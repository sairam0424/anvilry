...
| `velite` | `^0.4.0` | dev | MDX → typed `.velite/` collections | `velite.config.ts:1`; `next.config.ts:15` (`import("velite").then(({ build }) => build({ watch: true, clean: false }))`); referenced by the `predev`/`build`/`content` scripts | Pulls `esbuild@0.25.12` + `sharp@0.35.3` + `terser` (`pnpm-lock.yaml:10303-10306`). `src/lib/content.ts:14` imports the **generated output** `"../../.velite"`, not the package |
