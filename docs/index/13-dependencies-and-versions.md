...
| `@modelcontextprotocol/sdk` | `1.26.0` (EXACT, no caret) | prod | Peer requirement of `mcp-handler` (`mcp-handler` peer is literally `1.26.0`) | **NO DIRECT IMPORT FOUND** | Searched `src/`, `e2e/`, `scripts/`, all root config files, and the whole repo excluding `node_modules`/`.next`/`.velite`/`.git` — the only matches are `package.json:27` and `CHANGELOG.md:180,197`. Declared because pnpm requires the peer be installed; consumed only through `mcp-handler` (`pnpm-lock.yaml:8497`). The exact pin is why Dependabot returned `security_update_not_possible` |
