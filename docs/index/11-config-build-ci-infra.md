...
to the CI pin (`ci.yml:24-27` `node-version: 22`). `CHANGELOG.md:141-144` records the failure that motivated it: a
...
declared but imported nowhere, and it was **removed from `package.json` in v3.5.0** (`CHANGELOG.md:160-164`). The
...
**Gotcha:** `CHANGELOG.md:215-216` records that Dependabot reads `dependabot.yml` from the **default branch
...
**Why it moved, and why this counts as a security fix** (`pnpm-workspace.yaml:1-12`, `CHANGELOG.md:147-157` —
...
`body-parser` 2.3.0 · `brace-expansion` 1.1.18 and 5.0.9 (`CHANGELOG.md:153-157`).
...
both pnpm majors read — `CHANGELOG.md:160-161` records the removal as "deferred until the overrides migration
...
`CHANGELOG.md:174-198` (release 3.4.2, "resolves all 23 open Dependabot advisories across 10 packages, 11 of them
...
|---|---|---|
| `hono` | `^4.12.34` | 4.12.25 → 4.13.2 (medium ×3, low ×1): `memo()` retained SSR output across requests (cross-user data disclosure), ReDoS in CORS middleware, algorithmic-complexity DoS in Language middleware, Proxy Helper header leak (`CHANGELOG.md:181-183`) |
| `@hono/node-server` | `^1.19.15` | → 1.19.17 (medium): `serve-static` path traversal via `%5C` on Windows (`CHANGELOG.md:191`) |
| `ip-address` | `^10.3.1` | 10.2.0 → 10.5.0 (high): `Address4` decodes leading-zero octets as decimal while resolvers decode them as octal → SSRF / trust-boundary bypass. Reached via `mcp-handler` → `@modelcontextprotocol/sdk` → `express-rate-limit`; server-side only (`CHANGELOG.md:178-180`) |
| `fast-uri` | `^3.1.5` | 3.1.2 → 3.1.5 (high): host confusion via backslash authority introducer (`CHANGELOG.md:185`) |
| `js-yaml` | `^4.3.1` | 4.2.0 → 4.3.1 (high): quadratic CPU consumption in `!!omap` resolution (`CHANGELOG.md:184`) |
| `postcss` | `^8.5.23` | (medium/high): attacker-controlled `sourceMappingURL` read arbitrary `.map` files when `from` is unset (`CHANGELOG.md:192-193`) |
| `brace-expansion@1` | `^1.1.16` | (high) — the 1.x line, patched **within its line** |
| `brace-expansion@>=3` | `^5.0.7` | (high) — the 5.x line. Two version-scoped overrides instead of one blanket override: "A blanket override would have forced `minimatch@3` onto 5.x and broken the eslint chain" (`CHANGELOG.md:186-188`) |
| `sharp` | `^0.35.0` | 0.34.5 → 0.35.3 (high): "only velite's build-time instance was affected; Next 16.3.0 already carried the patched 0.35.3" (`CHANGELOG.md:189-190`) |
| `body-parser` | `^2.3.0` | 2.2.2 → 2.3.0 (low) (`CHANGELOG.md:194`) |
