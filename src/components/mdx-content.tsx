...
const components = {
  h2: (props: ComponentProps<"h2">) => (
    <h2
      className="mt-10 mb-3 text-xl font-semibold tracking-tight text-fg"
      {...props}
    />
  ),
  ...
  p: (props: ComponentProps<"p">) => (
    <p className="my-3 leading-relaxed text-fg-muted" {...props} />
  ),
  ul: (props: ComponentProps<"ul">) => (
    <ul className="my-4 space-y-1.5 pl-1 text-fg-muted" {...props} />
  ),
  li: ({ children, ...props }: ComponentProps<"li">) => (
    // The bullet dot is a flex item (`before:`); children must be its OWN single flex
    // item too, or flexbox blockifies every text run and inline child (e.g. `code`)
    // into separate boxes stacked instead of flowing as one sentence.
    <li
      className="flex gap-2 before:mt-2 before:h-1 before:w-1 before:shrink-0 before:rounded-full before:bg-accent"
      {...props}
    >
      <span>{children}</span>
    </li>
  ...
  code: (props: ComponentProps<"code">) => (
    <code
      className="rounded bg-bg-elevated px-1.5 py-0.5 font-mono text-[0.85em] text-violet"
      {...props}
    />
  ),
};
