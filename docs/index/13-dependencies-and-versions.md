...
| `mcp-handler` | `^1.1.0` | prod | `createMcpHandler` — wires 9 MCP tools to the `[transport]` route | `src/app/api/mcp/[transport]/route.ts:1,28` — only import site | Server-only. Pulls `redis@4.7.1`, `chalk`, `commander` (`pnpm-lock.yaml:8842-8844`); `disableSse: true` avoids its Redis init path (`route.ts:120-126`) |
...
`["esbuild"]` — pnpm 10 refuses to run install scripts unless a package is on this allowlist. `esbuild@0.25.12` is a **direct dependency of `velite@0.4.0`** (`pnpm-lock.yaml:10375-10378`) and needs its postinstall to fetch/link the platform binary, so content generation would fail without it. `vite@8.0.16` declares `esbuild: ^0.27.0 || ^0.28.0` as an *optional* peer (`pnpm-lock.yaml:4921,4936`) — that peer is unsatisfied by 0.25.12 and is not being relied on.
