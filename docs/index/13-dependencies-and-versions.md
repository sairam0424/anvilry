...
| `@react-three/drei` | `^10.7.8` | prod | R3F helpers (`useGLTF`, `Text`, `OrbitControls`, …) | `src/lib/r3f.ts:24` — **only** import site | Deliberately funnelled through the barrel. Carries `@types/three` as a transitive peer (`pnpm-lock.yaml:6267`) |
...
| `mcp-handler` | `^1.1.0` | prod | `createMcpHandler` — wires 9 MCP tools to the `[transport]` route | `src/app/api/mcp/[transport]/route.ts:1,28` — only import site | Server-only. Pulls `redis@4.7.1`, `chalk`, `commander` (`pnpm-lock.yaml:8844-8846`); `disableSse: true` avoids its Redis init path (`route.ts:120-126`) |
...
| `@modelcontextprotocol/sdk` | `1.26.0` | `mcp-handler@1.1.0`'s peer range is the single literal version `1.26.0` (`pnpm-lock.yaml:3865`). This pin is the direct cause of the `security_update_not_possible` failures |
