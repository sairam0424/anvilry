...
const stats = [
  ...
  {
    value: (allArticles.length + allNotes.length).toString(),
    label: "Articles & notes",
    sub: "published writing",
  },
    ...
    <main className="flex-1">
      <Section label="// numbers" title="By the numbers" titleAs="h1">
        <p className="mb-10 max-w-2xl text-fg-muted">
          A snapshot of open-source work, production systems, and engineering
          output.
        </p>
          ...
          {stats.map((s, i) => (
            <Reveal key={`${s.label}-${s.sub}`} delay={(i % 3) * 0.06}>
              <div className="card-surface flex flex-col gap-1 p-6">
                {/* Literal spaces: give Pagefind's static-HTML excerpt extraction word
                    boundaries between these three adjacent spans (e.g. "5,327Total
                    commitsacross all repos" with no spaces) — invisible visually,
                    Flexbox ignores whitespace-only text nodes for layout. */}
