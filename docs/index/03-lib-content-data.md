...
- **Gotchas / invariants:** The hardcoded `BASE` here is one of the base-URL sites — `https://anvilry.vercel.app` is hardcoded in **19 files / 25 occurrences**, not the four this index once claimed (`CLAUDE.md:335`, which now says so explicitly and gives the grep). `resume-json.ts:4` and `mcp-tools.ts:14` are two of the sibling lib copies. Run `grep -rn 'anvilry\.vercel\.app' src Makefile` rather than trusting any enumeration.
