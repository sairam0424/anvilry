...
| `@react-three/drei` | `^10.7.8` | prod | R3F helpers (`useGLTF`, `Text`, `OrbitControls`, …) | `src/lib/r3f.ts:24` — **only** import site | Deliberately funnelled through the barrel. Carries `@types/three` as a transitive peer (`pnpm-lock.yaml:6490`) |
...
| `postprocessing` | `^6.39.4` | prod | `BlendFunction` enum (peer of `@react-three/postprocessing`) | `src/components/chat/voice-orb-3d.tsx:5` — only import site | Client-bundled. **Peer constraint `three: >= 0.168.0 < 0.186.0`** (`pnpm-lock.yaml:4276`) — this is the ceiling on the `three` bump |
...
| `velite` | `^0.4.0` | dev | MDX → typed `.velite/` collections | `velite.config.ts:1`; `next.config.ts:15` (`import("velite").then(({ build }) => build({ watch: true, clean: false }))`); referenced by the `predev`/`build`/`content` scripts | Pulls `esbuild@0.25.12` + `sharp@0.35.3` + `terser` (`pnpm-lock.yaml:10326-10328`). `src/lib/content.ts:14` imports the **generated output** `"../../.velite"`, not the package |
| `vitest` | `^5.0.0` | dev | Unit/DOM test runner; chained into `pnpm build` | `vitest.config.ts:1` (`vitest/config`); `src/lib/llm.test.ts`, `src/app/api/error/route.test.ts:1`, `src/app/api/tts/cache.test.ts:1`, `src/app/layout.hydration-proof.dom.test.tsx:1` (+all `*.test.*`) | **Major bump 4.1.11 → 5.0.0.** Still resolves **Vite 8.0.16** (`pnpm-lock.yaml:10365`) — the major bump did not force a vite bump. `test.projects` workspace-in-config API (used by `vitest.config.ts`) is unaffected — full `pnpm build` chain (velite + vitest + next build + pagefind) and Playwright e2e both pass clean. Unexpected bonus: this bump appears to fix a local-only `localStorage`-in-happy-dom failure (Node 26.7, previously seen on `site-footer.dom.test.tsx` + `voice-pitfalls.dom.test.ts`) |
...
**Three.js ceiling.** `three` is `^0.185.1`, but `postprocessing@6.39.4` declares `three: >= 0.168.0 < 0.186.0` (`pnpm-lock.yaml:4276`). A bump to 0.186.x therefore breaks that peer, and the `three-webgl` Dependabot group batches `three`, `@react-three/*`, `@types/three`, and `postprocessing` together so the constraint is reviewed as a unit.
