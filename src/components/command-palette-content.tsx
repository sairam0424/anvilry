...
export function CommandPaletteContent({
  ...
  const views: Action[] = [
    {
      id: "v-classic",
      label: "Classic view",
      hint: "the standard portfolio",
      icon: <LayoutGrid size={16} />,
      run: () => switchTo("classic"),
      keywords: "default standard reset",
    },
    {
      id: "v-play",
      label: "Play view",
      hint: "explorable build graph",
      icon: <Gamepad2 size={16} />,
      run: () => switchTo("gamified"),
      keywords: "game gamified graph explore",
    },
    {
      id: "v-dev",
      label: "Developer mode",
      hint: "full-page terminal — query my work",
      icon: <TerminalSquare size={16} />,
      run: () => {
        track("devmode_palette_open");
        switchTo("developer");
      },
      keywords: "terminal cli console command shell whoami grep developer",
    },
    {
      id: "v-chat",
      label: "Chat view",
      hint: "ask my portfolio",
      icon: <MessagesSquare size={16} />,
      run: () => switchTo("chat"),
      keywords: "ai concierge conversation assistant",
    },
    {
      id: "v-resume",
      label: "Recruiter view",
      hint: "scan-friendly CV + PDF downloads",
      icon: <User size={16} />,
      run: () => switchTo("resume"),
      keywords: "resume cv recruiter scan pdf download hiring",
    },
    ...(discoveryBadgesEnabled
      ? [
          {
            ...
            keywords: "discovery badge explore unlock all",
          },
        ]
      : []),
  ];
  ...
  const links: Action[] = [
    {
      id: "gh",
      label: "GitHub",
      hint: `github.com/${profile.githubUser}`,
      icon: <Github size={16} />,
      run: () => go(profile.links.github, true),
    },
    {
      id: "li",
      label: "LinkedIn",
    ...
    },
    {
      ...
      hint: profile.email,
      icon: <Mail size={16} />,
      run: () => go(`mailto:${profile.email}`, true),
    },
    {
      id: "calendly",
      label: "Schedule a call",
      hint: "30-min intro call on Calendly",
      icon: <CalendarDays size={16} />,
      run: () => go(profile.calendlyUrl, true),
      keywords: "calendly meeting call schedule book chat",
    },
  ];
  ...
  const actions: Action[] = [
    ...
    {
      id: "mcp",
      label: "Open MCP endpoint",
      hint: "for AI agents",
      icon: <Plug size={16} />,
      run: () => go("/mcp"),
      keywords: "ai agent model context protocol tools api",
    },
  ];
  ...
  const voice: Action[] = [
      ...
      ? [
          {
            ...
            keywords:
              "voice pick choose select stephen joanna ruth matthew aoede charon catalog",
            value:
              "Pick voice choose select stephen joanna ruth matthew aoede charon catalog",
          },
          {
            id: "voice-settings",
            ...
            keywords:
              "voice settings preferences engine character speed tone pause toggles all-in-one",
            value:
              "Voice settings preferences engine character speed tone pause toggles all-in-one",
          },
        ]
      ...
      ? [
          {
            ...
            keywords:
              "voice talk conversation speak hands-free microphone mic chat assistant",
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            keywords:
              "voice speak audio tts text to speech accessibility listen sound",
            // Label flips with state, so pin a stable search value (cmdk re-scores on change).
            value:
              "Read answers aloud voice speak audio tts accessibility listen sound",
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            run: () => {
              set({
                ttsEngine: settings.ttsEngine === "polly" ? "browser" : "polly",
              });
              setOpen(false);
            },
    ...
    // AudioContext) may still work, so this is how a Firefox visitor enables voice.
    {
      ...
      run: () => {
        set({
          sttEngine:
            settings.sttEngine === "transcribe" ? "browser" : "transcribe",
        });
        setOpen(false);
      },
      keywords:
        "voice mic stt speech recognition transcribe aws privacy firefox input",
      value:
        "Mic speech recognition engine transcribe aws browser privacy input",
    },
      ...
      ? [
          {
            ...
            hint:
              settings.talkSurface === "view"
                ? "use the full Voice view"
                : "Talk pill + ⌘K",
            icon: <AudioLines size={16} />,
            run: () => {
              set({
                talkSurface: settings.talkSurface === "view" ? "modal" : "view",
              });
              setOpen(false);
            },
            keywords:
              "voice talk modal shortcuts pill command palette quick door visibility",
            value:
              "Voice modal shortcuts talk pill command palette quick door visibility",
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            label: settings.wakeWord
              ? "Turn off wake word"
              : "Enable wake word (Hey portfolio)",
            ...
            keywords:
              "wake word hey portfolio hands-free always listen hotword voice activate",
            value:
              "Enable wake word hey portfolio hands-free always listen hotword voice",
          },
        ]
      : []),
  ];
  ...
  const recentItems: Action[] = recent
    .map((id) => byId.get(id))
      ...
      >
        ...
        <div className="relative z-10 w-full max-w-lg overflow-hidden rounded-xl border border-border-strong bg-bg-surface shadow-2xl">
          ...
          <Command.List className="max-h-[50vh] overflow-y-auto p-2">
            ...
            {voice.length > 0 && (
              <Group heading="Voice" actions={voice} onRun={record} />
            )}
    ...
    >
      {actions.map((a) => (
        ...
        >
          ...
          {a.hint && (
            <span className="truncate text-xs text-fg-subtle">{a.hint}</span>
          )}
