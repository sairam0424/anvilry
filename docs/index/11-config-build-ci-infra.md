...
| `NEXT_PUBLIC_GRAPH_PHYSICS` | No (default off) | `src/components/hero-graph/index.tsx:8` | `"true"` swaps the lazy hero-graph chunk from `./scene` to `./scene-physics`'s `HeroGraphScenePhysics` (`src/components/hero-graph/index.tsx:15-17`) | **Not Rapier physics.** `scene-physics.tsx` wraps `HeroGraphInner` in a `<group>` and *sets* `position.x/y/z` from `Math.sin`/`Math.cos(clock.elapsedTime)` each `useFrame` (`src/components/hero-graph/scene-physics.tsx:37-45`); its own header says "No RigidBody / Rapier needed for this effect" (`:10-16`). `@react-three/rapier` is declared (`package.json:29`) but imported nowhere — "NO DIRECT IMPORT FOUND" ([13 § `dependencies` (35)](./13-dependencies-and-versions.md#dependencies-35), the `@react-three/rapier` row). The call-site comment used to claim a "Rapier physics variant is loaded"; **that misleading comment has since been corrected** — it now says "a drift variant is loaded instead of the static scene" and states outright that "despite the flag name, there is no physics engine involved … `@react-three/rapier` is declared in package.json but imported nowhere in src/; the flag and filename are historical" (`src/components/hero-graph/index.tsx:10-14`). Only the flag name and the filename remain historical; the comments no longer mislead. Undocumented in `.env.example` (listed in `ARCHITECTURE.md:99`) |
...
| `VERCEL_GIT_COMMIT_SHA` | Platform | `src/app/api/cron/health-check/route.ts:174` | `release_id` in the health-check payload (typed `release_id: string \| null` at `:43`) | — |
