...
- **Gotchas / invariants:** `BASE` is one of the hardcoded base URLs (`:6`). **Undercount — was a live defect, now FIXED:** `CLAUDE.md` used to list only four files to update on a custom domain (`layout.tsx`, `sitemap.ts`, `robots.ts`, `json-ld.tsx`), and this file's `BASE` was one of the occurrences missing from it. `CLAUDE.md:325` now says the host is hardcoded in **19 files / 25 occurrences**, "not four", and `src/app/feed.xml/route.ts` is named explicitly in the enumeration (`CLAUDE.md:335`); re-verify with `grep -rn 'anvilry\.vercel\.app' src Makefile` rather than trusting any list. Cross-reference: [15 § The hardcoded base URL](./15-invariants-and-gotchas.md#the-hardcoded-base-url). The feed ignores `NOTES_ENABLED` / `ARTICLES_ENABLED`, unlike `sitemap.ts`.
