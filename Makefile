...
env-check: ## Show which environment variables are set vs unset (secrets masked)
	...
	@echo ""
	@printf "$(YELLOW)FAQ Cache$(RESET)\n"
	@printf "  FAQ_CACHE_ENABLED           = %s\n" "$${FAQ_CACHE_ENABLED:-true (default) — set to 'false' as a kill switch}"
	@printf "  FAQ_CACHE_SEMANTIC_MATCH    = %s\n" "$${FAQ_CACHE_SEMANTIC_MATCH:-false (default) — set to 'true' to enable the embedding-similarity tier}"
	@echo ""
