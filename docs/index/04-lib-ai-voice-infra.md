...
- **Role:** Hardcoded, deterministic multi-agent trace script for the glass-box demo, plus the shipping gate that keeps it dark until owner-approved.
- **Exports:** `PLACEHOLDER_SENTINEL` = `"[DRAFT — owner to approve]"` (agent-trace.ts:19); `AgentName` (type) = `"Researcher" | "Synthesizer" | "Presenter"`; `AGENTS` (record of label/role/color, agent-trace.ts:24-28); `AgentStep`, `Scenario` (types); `scenarios` (two scenarios, `"scaling-aava"` and `"event-driven"`, three steps each, agent-trace.ts:49-104); `allReferencedSlugs()`; `traceApproved` (boolean const); `linkForSlug(slug)`.
...
- **Behaviour notes:** `traceApproved` is computed at module load — true only when no step's `action` or `output` contains `PLACEHOLDER_SENTINEL` (agent-trace.ts:117-119). Every step in the shipped file still carries the sentinel, so `traceApproved` is currently **false** and the demo renders dark. Agent colors are CSS custom properties (`var(--accent)`, `var(--violet)`, `var(--green)`), not hex. `AGENTS` colors and `ms` reveal delays (550–900) are the coordination-feel budget documented as ~400–1200/step, total < 8000 (agent-trace.ts:39).
- **Gotchas / invariants:** `agent-trace.test.ts` (4 assertions) is the zero-fabrication gate — CLAUDE.md:309 states it blocks shipping while the sentinel is present, and it asserts every `refs` slug resolves to real content. Referenced slugs across scenarios: `aava-code`, `mindforge`, `pensieve`. `traceApproved` is explicitly *not* a hard build failure (agent-trace.ts:111-115) — the dark render is the soft gate; the test is the hard one.
...
- **Gotchas / invariants:** All five routes share one prefix and one budget — they are *not* independently budgeted. `clientIp` is duplicated verbatim in `telemetry/with-trace.ts:63-73` on purpose (see that entry); a fix to one must be mirrored. Note there is no `rate-limit.test.ts` in the repo.
