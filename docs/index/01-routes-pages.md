...
**Files indexed:** 37 — matches the Coverage list below. It was 35: two in-scope regression guards were
added on this branch, `src/app/manifest.test.ts` and `src/app/mcp/tools-documented.test.ts`.
...
| `src/app/mcp/page.tsx` | `/mcp` — MCP server docs: endpoint, Claude Desktop + Cursor JSON configs, **10**-row tool table (all 10 registered; was 7, then 9 — see FIXED note in Detail) | `metadata`, default `McpPage` |
...
| `/work/[slug]` | `src/app/work/[slug]/page.tsx` | PARTIALLY_STATIC per-slug prerender + dynamic fallback | none | **yes** — `allWork` (`work/[slug]/page.tsx:13`) | **yes** (`work/[slug]/page.tsx:35`) | none | `getWork(slug)`, `notFound()` on miss |
...
| `/manifest.webmanifest` | `src/app/manifest.ts` | static metadata route | none | no | n/a | none | `profile`; icon routes `/icon` + `/apple-icon` (`manifest.ts:14`) — the two `/static/screenshot-*.png` entries were removed, see FIXED note below |
...
- **Gotchas / invariants:** `siteUrl` is **hardcoded** at `:26`; `CLAUDE.md:335` names this file as one of **19 files / 25 occurrences** to change for a custom domain and tells you to run `grep -rn 'anvilry\.vercel\.app' src Makefile` rather than trust the list. (CLAUDE.md previously claimed exactly four files — `layout.tsx`, `sitemap.ts`, `robots.ts`, `json-ld.tsx` — which was wrong and has been corrected on this branch; `json-ld.tsx` alone holds 7 occurrences, `CLAUDE.md:343`.) The `<a href="#main-content" className="skip-link">` at `:94` must stay the first focusable element (WCAG 2.4.1) and pairs with `#main-content tabIndex={-1}` at `:100`. Removing a `suppressHydrationWarning` re-opens the bug that `layout.hydration-proof.dom.test.tsx` locks down.
...
- **Behaviour notes:** renders its **own** `<html>`/`<body>` (`:84-85`) because the root layout has already unwound. Zero `@/components` imports — no `Providers` context exists at this point (`:11-13`). All load-bearing styling is inline `style` objects — **no longer duplicated hex** (fixed on this branch): every value now comes from the shared `METADATA_COLORS` constant (`:32` import), since `globals.css` may not have been applied yet (`:13-17`). Beacons `source:"global-boundary"` (`:63`) so the sink can distinguish a layout crash from a route crash.
- **Gotchas / invariants:** the "Back to homepage" link is a **plain `<a>`**, not `next/link`, with `@next/next/no-html-link-for-pages` disabled on purpose — the router context is part of what unwound, so a full navigation is the safer recovery (`:169-176`). **No longer a hand-maintained hex copy** (fixed on this branch, see the `METADATA_COLORS` row in `15-invariants-and-gotchas.md`) — every value here is `METADATA_COLORS.*`, mirroring `globals.css:8-36` by name.
...
- **Gotchas / invariants:** the glitch animation class `.glitch-eyebrow` is defined in `globals.css:404-420` and only there. It renders inside the root layout, so all `Providers` are available (`:20`). `cd /` in the terminal is the documented in-terminal escape hatch, surfaced as visible copy at `:84-85`.
...
- **Gotchas / invariants:** `SOURCE_LABELS` (`:25-32`) must stay in sync with the Velite `source` enum `["medium","substack","linkedin","devto","hashnode","native"]` (`velite.config.ts:109`) — a new platform without a label renders `undefined` in the filter pill.
...
- **Gotchas / invariants:** those two hardcoded figures are the only numbers on this page not derived from a single source — `CLAUDE.md:365` / `ARCHITECTURE.md:95` require metrics to be real and non-fabricated, so they must track the `impactMetrics` in `@/lib/profile` by hand.
...
- **Behaviour notes:** `ENDPOINT = "https://anvilry.vercel.app/api/mcp/mcp"` is a module constant (`:6`) interpolated into both the Claude Desktop config (`npx -y mcp-remote <ENDPOINT>`, `:17-24`) and the Cursor config (`{ "url": <ENDPOINT> }`, `:26-30`). The `TOOLS` array (`:40-60`) lists **all 10** tools registered by `src/app/api/mcp/[transport]/route.ts` — ten `server.registerTool(` calls at `route.ts:20`, `:30`, `:40`, `:49`, `:59`, `:68`, `:78`, `:88`, `:98`, `:109`, matching the "10 read-only tools" docblock at `route.ts:12` (see [02 § `src/app/api/mcp/[transport]/route.ts`](./02-api-routes.md#srcappapimcptransportroutets)).
  - **Was — FIXED on this branch:** the array hand-listed only **7 of the 9**; `list_all_content` (`route.ts:98`) and `get_content_item` (`route.ts:108`) had no row, so the public docs page under-reported the server and the two content tools were live but undiscoverable to anyone reading the docs. Both rows now exist (`:43-44`), and the drift is guarded — see Gotchas.
- **Gotchas / invariants:** `TOOLS` is still a **hand-maintained** list, not a derivation from `src/lib/mcp-tools.ts` — but it is no longer unguarded, so the drift above cannot silently recur. `src/app/mcp/tools-documented.test.ts` reads both this file's `TOOLS` block and the `server.registerTool(...)` names in `src/app/api/mcp/[transport]/route.ts` **from source** (neither is importable without widening a module-private surface) and asserts the two sets are identical: no undocumented registration (`:81-88`), no documented tool the route does not register (`:76-79`), equal counts and equal sorted sets (`:90-93`), plus a regex-drop cross-check so a name the pattern misses fails loudly instead of reading as "already documented" (`:61-74`). `vitest run` is chained into `pnpm build`, so adding a tool without a `TOOLS` row now **fails the build** with the missing names in the message. Segment config: this file exports **no** `dynamic`/`revalidate`, and `CLAUDE.md:140` now agrees — it documents `/mcp` as "no segment config"; the older `(force-static)` wording that this index used to flag as a contradiction is gone. A `force-static` grep over `src/app` still returns no non-API file. (14b records the same resolution: [`14b-subsystems.md:42-44`](./14b-subsystems.md).)
...
- **Gotchas / invariants:** `CLAUDE.md:365` records that three graph node IDs intentionally differ from these slugs (`aava` → `aava-code`, `grpc` → `grpc-microservices`, `nhl` → `not-humans-lab`) and that `game-model.test.ts` blocks deploys on orphaned nodes — renaming a work slug breaks that bijection test.
...
- **Gotchas / invariants:** `/notes`, `/articles`, `/stats`, `/search` are absent from the hardcoded static list on purpose — they are added conditionally further down. `base` is hardcoded (`:5`) — one of the **19** custom-domain edit points now enumerated at `CLAUDE.md:335-351` (that passage previously said four).
...
- **Gotchas / invariants:** filename must keep the `.dom.test.` infix — `CLAUDE.md:370` records that Vitest runs a `node` project for `*.test.ts` and a separate `dom` (happy-dom) project for `*.dom.test.*`, and that `NODE_ENV` is forced to `"test"` to avoid React's missing-`act()` warning.
...
- **`/stats` and `/search` are NOT route-gated.** `STATS_ENABLED` and `SEARCH_ENABLED` are consumed only by `src/app/sitemap.ts:80-88` and `src/components/site-nav.tsx:22-23`. Neither `stats/layout.tsx` nor `search/layout.tsx` calls `notFound()`, so both routes render and are prerendered even with the flags off — they are merely unlinked and un-sitemapped. Contrast `/articles` (gated in the layout, `articles/layout.tsx:12`) and `/notes` (gated in the page, `notes/page.tsx:21`).
- **Hardcoded base URL.** `"https://anvilry.vercel.app"` appears literally in `layout.tsx:26`, `sitemap.ts:5`, `robots.ts:6`, `mcp/page.tsx:6`, and as `const BASE` in `articles/[slug]/page.tsx:13`, `notes/[slug]/page.tsx:13`, `projects/[slug]/page.tsx:12`, `work/[slug]/page.tsx:11`. `CLAUDE.md:335` now scopes this correctly — **19 files / 25 occurrences**, "not four" — and enumerates the sites at `:333-339`, including all four `BASE` constants (`:336`) and `mcp/page.tsx` (`:335`). The old "four files for a domain change" claim (which this note was written to contradict) was corrected on this branch; the authoritative check is the grep at `CLAUDE.md:338`.
- **`cacheComponents` migration scars.** `work/page.tsx:9-13`, `notes/page.tsx:9-11`, `projects/page.tsx:31-36`, `notes/[slug]/page.tsx:34-42`, and `admin/telemetry/page.tsx:4-11` + `:428-433` each carry an in-file comment explaining a construct that was removed or added because `next.config.ts:183` sets `cacheComponents: true`. These comments are the only record of why `export const revalidate` is absent.
...
**Was**: `src/app/manifest.ts` declared two PWA `screenshots` at `/static/screenshot-desktop.png` (1280×800, `form_factor:"wide"`) and `/static/screenshot-mobile.png` (390×844, `form_factor:"narrow"`) while `public/static/` was an **empty directory** with no file matching `*screenshot*` anywhere under `public/` — so both URLs 404'd in every environment.

**Now**: the `screenshots` key is **deleted**. `manifest()` returns no `screenshots` at all; `icons` (`src/app/manifest.ts:14`) is the only asset list it declares, and every entry there is a Next.js generated route (`/icon`, `/apple-icon`). A comment at `src/app/manifest.ts:19-24` records the removal and the re-add contract (drop the PNGs into `public/static/` first). `public/static/` is still an empty directory, which is now consistent with the manifest rather than contradicted by it.
