...
| `mcp-handler` | `^1.1.0` | prod | `createMcpHandler` — wires 9 MCP tools to the `[transport]` route | `src/app/api/mcp/[transport]/route.ts:1,28` — only import site | Server-only. Pulls `redis@4.7.1`, `chalk`, `commander` (`pnpm-lock.yaml:8979-8981`); `disableSse: true` avoids its Redis init path (`route.ts:120-126`) |
