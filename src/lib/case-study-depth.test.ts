  ...
  it("every work item with a diagram has non-empty alt text (a11y)", () => {
    for (const w of allWork) {
      if (w.diagram) {
        expect(
          w.diagramAlt,
          `${w.slug}: diagram set but diagramAlt missing`,
        ).toBeTruthy();
  ...
  it("every referenced diagram asset exists on disk (no broken image)", () => {
    for (const w of allWork) {
      if (w.diagram) {
        ...
        expect(
          existsSync(onDisk),
          `${w.slug}: diagram asset not found at public/${rel}`,
        ).toBe(true);
      }
    }
  });
  ...
  it("constraints/tradeoffs (when present) are non-trivial prose, not a placeholder", () => {
    for (const w of allWork) {
      for (const field of ["constraints", "tradeoffs"] as const) {
        const val = w[field];
        if (val) {
          expect(val.length, `${w.slug}.${field} too short`).toBeGreaterThan(
            20,
          );
          expect(
            /TODO|TBD|lorem/i.test(val),
            `${w.slug}.${field} looks like a placeholder`,
          ).toBe(false);
        }
      }
    }
  ...
  it("every project's decisions (when present) are non-trivial prose, not a placeholder", () => {
    for (const p of allProjects) {
      for (const d of p.decisions) {
        expect(
          d.title.length,
          `${p.slug}: decision title too short`,
        ).toBeGreaterThan(3);
        expect(
          d.body.length,
          `${p.slug}: decision "${d.title}" body too short`,
        ).toBeGreaterThan(20);
        expect(
