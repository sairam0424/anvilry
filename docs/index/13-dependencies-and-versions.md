...
| `@react-three/drei` | `^10.7.8` | prod | R3F helpers (`useGLTF`, `Text`, `OrbitControls`, …) | `src/lib/r3f.ts:24` — **only** import site | Deliberately funnelled through the barrel. Carries `@types/three` as a transitive peer (`pnpm-lock.yaml:6443`) |
