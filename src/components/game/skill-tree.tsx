...
// ─── Color palette ────────────────────────────────────────────────────────────
// "Cloud & Ops" and "Frontend" are a separate qualitative categorical palette with
// no matching semantic token — they stay hardcoded. The other four groups mirror
// --accent/--violet/--green/--amber and are resolved live via useThemeColors()
// inside the component (see groupColors in SkillTree below), not here.
const STATIC_GROUP_COLORS: Record<string, string> = {
  "Cloud & Ops": "#f87171",
  Frontend: "#38bdf8",
};
...
function connectionPath(
  ...
  const cp1y = y0 + dy * 0.4;
  const cp2x = x1 - dx * 0.25;
  const cp2y = y1 - dy * 0.4;
  return (
...
function buildRowColumns(
  ...
  const rawCols = groupNames.map((groupName) => {
    ...
    const color = groupColors[cat.group] ?? DEFAULT_COLOR;
  ...
  const totalW =
    rawCols.reduce((sum, c) => sum + c.colW, 0) +
    GAP_BETWEEN * (rawCols.length - 1);
...
function buildColumns(groupColors: Record<string, string>): {
  columns: Column[];
  svgWidth: number;
  svgHeight: number;
} {
  const { columns: row1, rowBottom: row1Bottom } = buildRowColumns(
    ROW1_GROUPS,
    CAT_Y_ROW1,
    groupColors,
  );
  ...
  const { columns: row2, rowBottom: row2Bottom } = buildRowColumns(
    ROW2_GROUPS,
    catYRow2,
    groupColors,
  );
      ...
      <rect
        ...
        style={{
          transition: reducedMotion
            ? undefined
            : "fill-opacity 0.15s, stroke-opacity 0.15s",
        }}
      />
      {/* Label */}
      <text
...
export function SkillTree() {
  ...
  const groupColors: Record<string, string> = {
    Languages: themeColors.accent,
    GenAI: themeColors.violet,
    "Backend & Distributed": themeColors.green,
    "Data & Messaging": themeColors.amber,
    ...STATIC_GROUP_COLORS,
  };
  const { columns, svgWidth, svgHeight } = buildColumns(groupColors);
      ...
      >
          ...
          return (
            <g
              ...
              style={{
                ...
                filter:
                  isActive && !reducedMotion ? `url(#${hexGlowId})` : undefined,
              }}
            >
              ...
              {[0, 1, 2, 3, 4, 5].map((i) => {
                ...
                const by =
                  col.catY + Math.sin(angle) * innerR * (CAT_H / CAT_W);
                const tx = col.catX + Math.cos(angle) * outerR;
                const ty =
                  col.catY + Math.sin(angle) * outerR * (CAT_H / CAT_W);
                return (
                  <line
                    key={i}
                    x1={bx.toFixed(1)}
        ...
        {(() => {
          const row1Bottom = Math.max(
            ...columns
              .filter((c) => ROW1_GROUPS.includes(c.group))
              .map((c) => c.height),
          );
          const row2Top = Math.min(
            ...columns
              .filter((c) => ROW2_GROUPS.includes(c.group))
              .map((c) => c.catY - CAT_H / 2),
          );
