...
| Dependencies | **35 prod + 18 dev** (was 35 prod at v3.4.2 — v3.5.0 removed `@react-three/offscreen` and `@react-three/rapier`, both declared but never imported; 2 more prod deps and 1 more dev dep were added since), plus 10 `overrides`, 1 `onlyBuiltDependencies`, and a 3-entry `allowBuilds` map. **There is no `pnpm` field in `package.json`:** v3.5.0 moved every pnpm setting to `pnpm-workspace.yaml`, because pnpm 11 stopped reading that field and would have silently dropped the ten security overrides | `package.json:24-58` (prod), `:61-78` (dev); `pnpm-workspace.yaml:18-28` (overrides), `:44-45` (`onlyBuiltDependencies`), `:55-58` (`allowBuilds`) — see [13](./13-dependencies-and-versions.md) |
