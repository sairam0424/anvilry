...
| `CHANGELOG.md` | Keep-a-Changelog release history, **18** released version entries, newest first, plus a live `[Unreleased]` section (`CHANGELOG.md:7`) holding the pnpm 11 `allowBuilds` fix. The previous `[Unreleased]` block was cut as `[3.5.0] — 2026-08-21` (`CHANGELOG.md:62-164`). | `[3.5.0]`, `[3.4.2]` … `[1.0.0]`, link-ref footer |
...
| `CHANGELOG.md` | release log | Per-release narrative; 18 tags | latest entry `[3.5.0] — 2026-08-21` (`CHANGELOG.md:62`); rewritten in v3.5.0 |
...
(`CHANGELOG.md:62-164`), covering six live defects and the documentation corrections listed in
...
**Current version:** `3.5.0` (`package.json:3`). **What v3.5.0 shipped** (`CHANGELOG.md:62-164`): the six
defect fixes tabled above, the pnpm-settings migration to `pnpm-workspace.yaml` (`CHANGELOG.md:147` — pnpm 11
...
to `">=22 <23"` (`CHANGELOG.md:141`, `package.json:5-7`), and the removal of `@react-three/rapier` +
`@react-three/offscreen` (`CHANGELOG.md:160-164`: 3 packages removed, 0 added, and one version change —
...
**What v3.4.2 shipped** (`CHANGELOG.md:166-216`): a
...
1. ~~**Model-chain order in `DEPLOY.md` is inverted.**~~ — **corrected on this branch, both chains.**
   ...
   pointer at `DEPLOY.md:102-103`. (`CHANGELOG.md:540-542` records fixing the same inversion once already,
...
2. ~~**MCP tool count is 9, not 7.**~~ — **corrected on this branch, and guarded.** `CLAUDE.md` used to say
   ...
   `CHANGELOG.md:419` records the 7 → 9 growth at v3.0.0, and `CHANGELOG.md:83-85` records this doc fix
   landing in v3.5.0.
3. ~~**"Every API route runs on the Node.js runtime … with a 30s max duration" is doubly stale.**~~ —
   **corrected on this branch.** `CLAUDE.md` now leads the section with "**Runtime & duration — do not add
   ...
   noting its removal (`src/app/api/mcp/[transport]/route.ts:6-8`), consistent with `CHANGELOG.md:288-290`
...
11. **`CLAUDE.md`'s CI description omits two of `ci.yml`'s four jobs.** *(Still live — but re-measured on
    ...
    **four** jobs: `ci:` (`:10`), `e2e:` "E2E (Playwright)" (`:68-69`) running the new "Bundle budget" step
    ...
    `e2e` and `security-alerts` were added in v3.4.2 (`CHANGELOG.md:201-205`); `install-pnpm-11` is newer and
...
15. **Version-marker skew (not a code contradiction, but a freshness trap).** *(Still live.)*
    ...
    `CHANGELOG.md:822-830` has link references only for `1.0.0`–`1.6.0`; `1.7.0` and everything after — now
