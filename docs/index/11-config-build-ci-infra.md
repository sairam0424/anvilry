...
| `pnpm-workspace.yaml` | Not a workspace root. **The single source of truth for all pnpm settings as of v3.5.0**: 10 security `overrides` (`:18-28`), `onlyBuiltDependencies: [esbuild]` (`:32-33`), `ignoredBuiltDependencies: [sharp, unrs-resolver]` (`:36-38`). The 12-line header comment (`:1-12`) records why they moved out of `package.json` | n/a (YAML) |
...
| `.github/workflows/ci.yml` | 3 jobs: `ci` (lint/tsc/test **+ codebase-index citation check**), `e2e` (Playwright, build + run), `security-alerts` (non-blocking Dependabot report) | n/a |
...
| | | **`security-alerts`** (5 min, `continue-on-error: true`, `permissions: contents: read`): one step reading the Dependabot alerts API with `secrets.SECURITY_ALERTS_TOKEN`, writing a severity table + affected-package list to `$GITHUB_STEP_SUMMARY` | **Nothing — non-blocking by design** (`ci.yml:139`). Without the secret it prints setup instructions and `exit 0`. Documented token limitation: the default `GITHUB_TOKEN` **cannot** read that endpoint even with `security-events: read` — the restriction is on the token *type*, and a fine-grained PAT with `Dependabot alerts: Read-only` is required (`ci.yml:126-135`) |
