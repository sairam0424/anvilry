...
- **Tools wired (9):** `get_profile` (:30), `list_projects` (:40), `get_project` (:50, `T.projectSlugSchema`), `list_work` (:59), `get_work` (:69, `T.workSlugSchema`), `search_experience` (:78, `T.searchSchema`), `get_resume_variant` (:88, `T.resumeRoleSchema`), `list_all_content` (:98), `get_content_item` (:108, `T.contentTypeSchema`). All nine delegate to pure functions in `src/lib/mcp-tools.ts`.
- **Doc-count drift — was a live defect, now FIXED.** This route's own docblock and the public `/mcp` page both used to say "7 tools", omitting `list_all_content` and `get_content_item`, so two live tools were undiscoverable by anyone reading the docs. Current state: the docblock says "9 read-only tools" (:22), `src/app/mcp/page.tsx:35-45` lists all nine in its `TOOLS` table, and `CLAUDE.md:212` / `CLAUDE.md:303` both say 9. **Guard:** `src/app/mcp/tools-documented.test.ts` reads both files from source — the page's `TOOLS` rows and the route's `server.registerTool(` calls (`:32`) — and fails with the missing names when the two sets disagree (`:85`). Because `vitest run` is chained into `pnpm build`, the drift cannot silently return.
