...
- **Gotchas / invariants:** Model output reaches the DOM only as React text nodes; cards come from the slug allowlist (:266-272 doc). The container carries `[overflow-anchor:none]` to stop browser scroll-anchoring fighting the JS pin (:395-399). As of 2026-09-18, `useChatA11y` (`liveMessage`, rendered `aria-live="polite" aria-atomic="true"` at `:403`) is the ONLY deliberate `aria-live="polite"` announcer on this surface — both the transcript scroll container (`:448`) and `ThinkingBlock`'s live-reasoning `<pre>` (`:231`) explicitly carry `aria-live="off"`, each fixed after a real double-announce bug (transcript container: PR #257/#258; live-reasoning: PR #262, found via a live Playwright-MCP E2E sweep since the regression test's mock never exercised the thinking phase). Regression tests: `chat-messages.dom.test.tsx`, `chat-surface-live-region.dom.test.tsx`.
...
- **Consumed by:** `talk-mode.tsx:19` (`mode="dialog"`), `voice-settings-dialog.tsx:7` (`mode="inline"`), `command-palette-content.tsx:57` (rendered at :698).
