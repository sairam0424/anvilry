...
| `NEXT_PUBLIC_RESUME_VARIANTS` | v3.1 | Show 4 role-targeted PDF variants (Backend / Full-Stack / Frontend / GenAI) on the `/resume` page and in the `?view=resume` PDF downloads section. When unset, only the master "Sairam Resume" PDF is shown. Set `"true"` to enable (default OFF). Redeploy required. |
