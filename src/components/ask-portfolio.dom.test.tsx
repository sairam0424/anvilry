...
describe("AskPortfolio widget (unified onto useChat)", () => {
  it("streams an assistant answer through the shared transport", async () => {
    ...
    // Scoped to the transcript log: once settled, useChatA11y also mirrors the same
    // final text into the sr-only aria-live announcer (announce-on-settle, see
    // use-chat-a11y.ts), so an unscoped query matches both and throws. Timeout above
    // the default 1000ms: the commit is coalesced onto requestAnimationFrame with a
    // 250ms safety-timer fallback (BACKGROUND_FLUSH_MS in use-chat.ts) for when rAF
    // doesn't fire — under happy-dom + CI load that fallback can occasionally miss
    // the default window.
