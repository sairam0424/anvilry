...
- **Request → stream → fallback → telemetry path:**
  ...
  10. `streamWithFallback({ max_tokens: 1024, system: [{ …, cache_control: { type: "ephemeral" } }], messages }, { traceId, onError, onAttempt, extendedThinking })` (:265-315). Model chain is `us.anthropic.claude-sonnet-4-6` (or `-sonnet-5` when `LLM_USE_SONNET_5=true`) → `us.anthropic.claude-opus-4-6-v1` → `us.anthropic.claude-haiku-4-5-20251001-v1:0` (`src/lib/llm.ts:52-60`, `bedrockChain()`). Fallback is only attempted while zero bytes have been emitted; once `emittedAny` is true an apology tail is appended and the stream closes (`src/lib/llm.ts:540-544`). Extended thinking (migrated 2026-09-18 to adaptive thinking) is skipped entirely for Haiku and bumps `max_tokens` to ≥2048 with `thinking:{type:"adaptive"}` + `output_config:{effort:"low"}` — no more `budget_tokens` (`src/lib/llm.ts:348-349`, `:352-393`).
