...
describe("terminal command registry", () => {
  ...
  it("personal reveal commands print the REAL owner content (populated path)", () => {
    ...
    const secret = runCommand("secret")
      .lines.map((l) => l.text)
      .join("\n");
  ...
  it("`now` staleness line is honest: today / stale / NaN-guarded (fake timers)", () => {
    ...
    try {
      ...
      vi.setSystemTime(
        new Date(Date.parse(`${updated}T00:00:00Z`) + 91 * 86_400_000),
      );
      expect(
        runCommand("now")
          .lines.map((l) => l.text)
  ...
  it("unknown command fails closed with a hint", () => {
    ...
    expect(
      res.lines.some(
        (l) => l.kind === "err" && /command not found/.test(l.text),
      ),
    ).toBe(true);
  });
  ...
  it("normalizes input: trims, collapses inner whitespace, is case-insensitive on the name", () => {
    ...
    expect(
      padded.lines.some((l) => /COMMANDS|whoami|stack/i.test(l.text)),
    ).toBe(true);
    // command name is matched case-insensitively
    expect(
      runCommand("HELP").lines.some((l) =>
        /COMMANDS|whoami|stack/i.test(l.text),
      ),
    ).toBe(true);
    // multi-space between name and args still parses the args (open <slug>)
    const real = allWork[0].slug;
    expect(runCommand(`open    ${real}`).nav).toEqual({
      type: "route",
      href: `/work/${real}`,
    });
  });
  ...
  it("classic switches view", () => {
    expect(runCommand("classic").nav).toEqual({
      type: "view",
      view: "classic",
    });
  });
  ...
  it("every slug `ls` lists is also `cat`-able (cat resolves via the graph chain, ls via content — pin the contract)", () => {
    for (const item of [...allWork, ...allProjects]) {
      ...
      expect(failed, `cat ${item.slug} should resolve (ls lists it)`).toBe(
        false,
      );
    }
  });
  ...
  it("resume lists variants, opens a real one externally, rejects fake", () => {
    ...
    expect(runCommand("resume nope").lines.some((l) => l.kind === "err")).toBe(
      true,
    );
  });
  ...
  it("resume <substring> errors on unknown variant when flag OFF (only master visible)", () => {
    ...
    expect(
      runCommand("resume backend").lines.some((l) => l.kind === "err"),
    ).toBe(true);
  });
  ...
  it("whoami banner is grounded: every impactMetric value appears, no extra metric is invented", () => {
    ...
    for (const m of impactMetrics) {
      expect(
        banner,
        `metric value "${m.value}" must be in the banner`,
      ).toContain(m.value);
    }
    // ...and the derived repo count must equal the real project total (anti-drift).
    const repoMetric = impactMetrics.find(
      (m) => m.label === "open-source repos",
    );
    expect(repoMetric?.value).toBe(`${allProjects.length}`);
  });

  it("contact/email/social surface real profile links; email navs to mailto", () => {
    const contact = runCommand("contact")
      .lines.map((l) => l.text)
      .join("\n");
    expect(contact).toContain(profile.email);
    expect(contact).toContain(profile.links.github);
    // The contact box renders inside a fixed-width fmt.box, which truncates long
    // lines (unlike the plain-text `social` command below) — assert domain-level
    // substrings here rather than the full (possibly-truncated) URLs.
    expect(contact).toContain("npmjs.com");
    expect(contact).toContain("pypi.org");
    expect(contact).toContain("dev.to");
    expect(contact).toContain("substack.com");
    const social = runCommand("social")
      .lines.map((l) => l.text)
      .join("\n");
    expect(social).toContain(profile.links.npm);
    expect(social).toContain(profile.links.pypi);
    expect(social).toContain(profile.links.devto);
    expect(social).toContain(profile.links.substack);
    const em = runCommand("email");
    expect(em.lines.map((l) => l.text).join("\n")).toContain(profile.email); // selectable text first
    expect(em.nav).toEqual({
      type: "external",
  ...
  it("open routes to github/linkedin/npm/pypi/devto/substack/resume quick targets", () => {
    expect(runCommand("open github").nav).toEqual({
      type: "external",
      href: profile.links.github,
    });
    expect(runCommand("open linkedin").nav).toEqual({
      type: "external",
      href: profile.links.linkedin,
    });
    expect(runCommand("open npm").nav).toEqual({
      type: "external",
      href: profile.links.npm,
    });
    expect(runCommand("open pypi").nav).toEqual({
      type: "external",
      href: profile.links.pypi,
    });
    expect(runCommand("open devto").nav).toEqual({
      type: "external",
      href: profile.links.devto,
    });
    expect(runCommand("open substack").nav).toEqual({
      type: "external",
      href: profile.links.substack,
    });
    expect(runCommand("open resume").nav).toEqual({
      type: "route",
