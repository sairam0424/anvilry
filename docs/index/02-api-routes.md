...
- **Request → stream → fallback → telemetry path:**
  ...
  10. `streamWithFallback({ max_tokens: 1024, system: [{ …, cache_control: { type: "ephemeral" } }], messages }, { traceId, onError, onAttempt, extendedThinking })` (:265-315). Model chain is `us.anthropic.claude-sonnet-4-6` (or `-sonnet-5` when `LLM_USE_SONNET_5=true`) → `us.anthropic.claude-opus-4-6-v1` → `us.anthropic.claude-haiku-4-5-20251001-v1:0` (`src/lib/llm.ts:52-60`, `bedrockChain()`). Fallback is only attempted before any `text_delta` event has been received — `emittedAny` is set unconditionally the moment one arrives, even if the text strips to empty; once true, an apology tail is appended and the stream closes (`src/lib/llm.ts:563-590`, which as of PR #262 also closes any open thinking phase with `THINKING_END` first). Extended thinking (migrated 2026-09-18 to adaptive thinking) is skipped entirely for Haiku and bumps `max_tokens` to ≥2048 with `thinking:{type:"adaptive"}` + `output_config:{effort:"low"}` — no more `budget_tokens` (`src/lib/llm.ts:348-349`, `:352-393`).
...
- **Consumed by:** probed as `mcp_get` by `src/app/api/cron/health-check/route.ts:61` (`/api/mcp/mcp`).
...
- **Consumed by:** linked from `src/components/site-footer.tsx:183`; probed by `seo-audit/route.ts:30` and `health-check/route.ts:64`.
...
- **Gotchas / invariants:** `BASE` is one of the hardcoded base URLs (`:6`). **Undercount — was a live defect, now FIXED:** `CLAUDE.md` used to list only four files to update on a custom domain (`layout.tsx`, `sitemap.ts`, `robots.ts`, `json-ld.tsx`), and this file's `BASE` was one of the occurrences missing from it. `CLAUDE.md:335` now says the host is hardcoded in **19 files / 25 occurrences**, "not four", and `src/app/feed.xml/route.ts` is named explicitly in the enumeration (`CLAUDE.md:345`); re-verify with `grep -rn 'anvilry\.vercel\.app' src Makefile` rather than trusting any list. Cross-reference: [15 § The hardcoded base URL](./15-invariants-and-gotchas.md#the-hardcoded-base-url). The feed ignores `NOTES_ENABLED` / `ARTICLES_ENABLED`, unlike `sitemap.ts`.
...
- **Consumed by:** `/llms.txt` linked from `src/components/site-footer.tsx:81`, checked by `e2e/views.spec.ts:96`; both probed by `health-check/route.ts:62-63` with a **`< 1000` chars ⇒ fail** body-length assertion.
