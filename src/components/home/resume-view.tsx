...
export function ResumeView() {
  return (
    <main className="min-h-screen bg-bg-base px-4 py-10 text-fg print:bg-white print:text-black sm:px-8">
      <div className="mx-auto max-w-3xl">
        ...
        <header className="mb-8 border-b border-border pb-6 print:border-gray-300">
          ...
          <p className="mt-1 text-sm text-fg-subtle print:text-gray-600">
            {profile.location} ·{" "}
            <a href={`mailto:${profile.email}`} className="text-accent hover:underline print:text-black">
              {profile.email}
            </a>
            {" "}·{" "}
            <a href={profile.links.github} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline print:text-black">
              GitHub
            </a>
            {" "}·{" "}
            <a href={profile.links.linkedin} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline print:text-black">
              LinkedIn
            </a>
          </p>
          ...
          <div className="mt-4 flex flex-wrap gap-4">
            {impactMetrics.map((m) => (
              <div key={m.label} className="text-sm">
                ...
                {" "}<span className="text-fg-muted print:text-gray-600">{m.label} ({m.sub})</span>
              </div>
            ))}
          </div>
        </header>
        ...
        <section className="mb-8">
          ...
          <div className="space-y-4">
            {allWork.map((w) => (
              <div key={w.slug} className="rounded-lg border border-border p-4 print:border-gray-300">
                ...
                {w.metrics.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-3">
                    {w.metrics.map((met) => (
                      <span key={met.label} className="text-xs text-accent print:text-black">
                        {met.value} {met.label}
                      </span>
                    ))}
                  </div>
        ...
        <section className="mb-8">
          ...
          <div className="space-y-3">
            {allProjects.map((p) => (
              <div key={p.slug} className="rounded-lg border border-border p-3 print:border-gray-300">
                <div className="flex flex-wrap items-start justify-between gap-1">
        ...
        <footer className="border-t border-border pt-4 print:border-gray-300">
          <p className="text-sm text-fg-muted print:text-gray-600">
            ...
            {" "}or on{" "}
            <Link href={profile.links.linkedin} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline print:text-black">
              LinkedIn
            </Link>
          </p>
        </footer>
